...
class _HistoryDetailScreenState extends State<HistoryDetailScreen> {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // The scanned image preview — skipped for the demo seeder's solid
          // colour placeholders (they render as a useless flat block). Real
          // scans get a smaller preview that doesn't overpower the visual.
          if (!_isDemoPlaceholder(h.imagePath))
            ClipRRect(
              borderRadius: BorderRadius.circular(18),
              child: Image.file(
                File(h.imagePath),
  ...
  /// Demo seed entries write solid-colour PNGs into `<docs>/demo_scans/`.
  /// Detect that path so we can skip the giant flat block in the visualiser
  /// tab and let the animation be the focus.
  bool _isDemoPlaceholder(String path) {
    if (path.isEmpty) return true;
    return path.contains('/demo_scans/') || path.endsWith('/demo_scans');
  }
